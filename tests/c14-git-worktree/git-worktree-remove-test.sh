...
REMOVE_SCRIPT="$REPO_ROOT/skills/c14-git-worktree/scripts/git-worktree-remove.sh"
...
test_remove_uses_recorded_branch_after_switch() {
  ...
  assert_contains "$output" "C14 created branch: cxi/worktree/feature-a" "reported recorded branch"
...
test_remove_deletes_unmerged_recorded_branch() {
  ...
  assert_contains "$output" "C14 创建分支 cxi/worktree/feature-b 已删除" "unmerged temporary branch deletion"
...
test_remove_accepts_non_cxi_metadata() {
  ...
  metadata_file="$(git -C "$worktree" rev-parse --git-path c14-created-branch)"
  git -C "$repo" branch worktree-legacy
  printf '%s\n' 'worktree-legacy' > "$metadata_file"

  output="$(cd "$repo" && "$REMOVE_SCRIPT" "$worktree")"

  assert_contains "$output" "C14 创建分支 worktree-legacy 已删除" "non-cxi metadata deletion"
