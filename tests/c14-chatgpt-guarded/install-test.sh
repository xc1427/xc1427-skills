...
[[ "${short_version}" == "0.4" ]] || fail "unexpected launcher version: ${short_version}"
bundle_version="$(/usr/libexec/PlistBuddy -c 'Print :CFBundleVersion' "${destination}/Contents/Info.plist")"
[[ "${bundle_version}" == "4" ]] || fail "unexpected launcher build: ${bundle_version}"
...
assert_contains "${source_file}" 'let isGuarded = hasExpectedEnvironment(current)' "existing guarded process check missing"
