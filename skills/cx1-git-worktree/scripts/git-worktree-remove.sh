...
METADATA_FILE=""
if [ -d "$MATCH_PATH" ]; then
  METADATA_FILE=$(git -C "$MATCH_PATH" rev-parse --git-path cx1-created-branch 2>/dev/null || true)
  if [ -n "$METADATA_FILE" ] && [ -f "$METADATA_FILE" ]; then
    CREATED_BRANCH=$(head -n 1 "$METADATA_FILE")
    if ! git check-ref-format --branch "$CREATED_BRANCH" > /dev/null 2>&1; then
      echo "错误: CX1 创建分支元数据无效，已停止删除:" >&2
...
if [ -n "$CREATED_BRANCH" ]; then
  echo "CX1 created branch: $CREATED_BRANCH"
fi
echo ""
