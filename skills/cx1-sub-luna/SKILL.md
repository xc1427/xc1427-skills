---
name: cx1-sub-luna
description: 显式将用户当前指派的工作委派给一个 GPT-5.6 Luna 子代理。仅当用户明确调用 cx1-sub-luna 时使用；reasoning effort 默认 high，用户显式指定时覆盖。
---

# CX1 Spawn Agent by Luna

把用户当前指派的完整工作交给一个新的 Luna 子代理执行。

## 委派

1. 调用 `spawn_agent`，将 `model` 固定为 `gpt-5.6-luna`。
